...
const syncPlugin: esbuild.Plugin = {
	...
	setup(build) {
		build.onEnd(async () => {
			...
			await copy(join(__dirname, 'dist'), join(thisPluginDirectory, 'package', 'dist'));
			await copyFile(join(__dirname, 'package.json'), join(thisPluginDirectory, 'package', 'package.json'));

			// Copy .git to mark as locally installed plugin
			await copy(join(__dirname, '.git'), join(thisPluginDirectory, 'package', '.git'));
